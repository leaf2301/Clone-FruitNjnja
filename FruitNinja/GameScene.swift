...
//  FruitNinja
//
//  Created by Tran Hieu on 17/12/2023.
//
import AVFoundation
import SpriteKit
import GameplayKit
...
class GameScene: SKScene {
    ...
    func createScore() {
        ...
        gameScore.position = .init(x: 8, y: 8)
    ...
    func createLives() {
        for i in 0..<3 {
            ...
            spriteNode.position = .init(x: CGFloat(834+(i*70)), y: 720)
            addChild(spriteNode)
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        activeSliceBG.run(SKAction.fadeOut(withDuration: 0.05))
        activeSliceFG.run(SKAction.fadeOut(withDuration: 0.05))
    }
    ...
    func createEnemy(forceBomb: ForceBomb = .random) {
        ...
        if enemyType == 0 {
            ...
            if let emiter = SKEmitterNode(fileNamed: "sliceFuse") {
                emiter.position = .init(x: 76, y: 64)
                enemy.addChild(emiter)
            }
        } else {
